    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="relative w-full max-w-md overflow-hidden rounded-2xl p-8 glass-panel glow-border"
        >
          {/* Close button */}
          <button
            onClick={onClose}
          ...
          <div className="text-center mb-8">
            <h2 className="text-2xl font-bold text-white mb-2 glow-text">
              {mode === "login" ? "Welcome Back" : "Join LifeLink"}
            </h2>
            <p className="text-sm text-cyanGlow">
              {mode === "login"
                ? "Secure access to emergency medical resources."
                : "Register for immediate healthcare access."}
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              className="w-full py-3 mt-4 bg-cyanGlow text-deepBlue font-bold rounded-lg hover:bg-white hover:shadow-[0_0_15px_rgba(0,229,255,0.6)] transition-all"
            >
          ...
          <div className="mt-6 space-y-3">
            <button
              onClick={onSuccess}
              className="flex items-center justify-center w-full gap-3 py-3 border border-gray-600 rounded-lg text-sm text-white hover:bg-white/5 transition-all"
            >
